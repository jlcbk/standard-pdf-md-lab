...
| 正文准确性 | 2 | 文本相对少符号污染，但被切成大量短行，语义连续性很差 |
| 表格保真 | 1 | 表格结构基本丢失 |
| 图像处理 | 0 | 未输出图片或公式 assets |
| 噪声清理 | 1 | 页眉页脚、分页符和碎行非常明显 |
...
- 未提取图片、公式或图形。
- 输出包含分页控制字符和重复页眉。
- 正文中标点、单位、类别符号被拆散，例如 `M2`、`M3`、`N`、`O` 类上下文分离。
- 表格内容退化为逐行文本，无法可靠还原行列关系。
...
- 建议：淘汰为主方案，仅保留为对照组。
- 原因：它能成功抽文本，但结构化 Markdown 能力不足，后处理成本明显过高。
